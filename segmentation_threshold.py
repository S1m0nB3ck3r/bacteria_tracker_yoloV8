...
def main():
    ...
    video_capture = cv2.VideoCapture(
        "/mnt/data10tb/DATASETS/BACTERIA/Videos/T1J (9h16-9h33).m4v"
    )
    ...
    while True:
        ...
        _, thresholded_frame_l = cv2.threshold(
            left, thresh_l, 255, cv2.THRESH_BINARY
        )  # 90
        _, thresholded_frame_r = cv2.threshold(
            right, thresh_r, 255, cv2.THRESH_BINARY
        )  # 90

        # contours, _ = cv2.findContours(thresholded_frame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        contours_l, _ = cv2.findContours(
            thresholded_frame_l, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        contours_r, _ = cv2.findContours(
            thresholded_frame_r, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )

        # noise removal
        kernel = np.ones((5, 5), np.uint8)
        # opening = cv2.morphologyEx(thresholded_frame, cv2.MORPH_OPEN, kernel, iterations=2)
        opening_l = cv2.morphologyEx(
            thresholded_frame_l, cv2.MORPH_OPEN, kernel, iterations=1
        )
        opening_r = cv2.morphologyEx(
            thresholded_frame_r, cv2.MORPH_OPEN, kernel, iterations=1
        )
        ...
        ret, sure_fg_l = cv2.threshold(
            dist_transform_l, 0.7 * dist_transform_l.max(), 255, 0
        )
        ret, sure_fg_r = cv2.threshold(
            dist_transform_r, 0.7 * dist_transform_r.max(), 255, 0
        )
        ...
        mask_l = np.zeros_like(
            thresholded_frame_l, dtype=np.uint8
        )  # thresholded_frame_l
        mask_r = np.zeros_like(
            thresholded_frame_r, dtype=np.uint8
        )  # thresholded_frame_l
        ...
        watershed_contours_l, _ = cv2.findContours(
            mask_l, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE
        )
        watershed_contours_r, _ = cv2.findContours(
            mask_r, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE
        )
        ...
        for contour in contours_l:
            ...
            with open(
                f"/mnt/data10tb/DATASETS/BACTERIA/auto_labeled/vid9/frame_{frame_number}.txt",
                "w",
            ) as label_file:
                label_file.write(
        ...
        for contour in contours_r:
            ...
            # Save frame as image
        cv2.imwrite(
            f"/mnt/data10tb/DATASETS/BACTERIA/auto_labeled/vid9/frame_{frame_number}.jpg",
            img,
        )
        frame_number += 1
